    ...
    <div className="relative w-full h-full">
      <div className="relative h-full w-full grid grid-cols-2 md:grid-cols-3 grid-rows-3 md:grid-rows-2 bg-black">
        {identities.map((identity) => (
          ...
          >
            {/* Main structural link for the entire tile */}
            <Link
              href={identity.href}
              onClick={onClose}
            ...
            <div className="absolute inset-0 z-20 flex flex-col items-center justify-center p-4 text-center bg-black/80 backdrop-blur-[2px] opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
              ...
              {identity.whatsappUrl ? (
                <a
                  ...
                  className="inline-flex items-center gap-1.5 text-xs px-3.5 py-1.5 rounded-full bg-green-600 hover:bg-green-500 text-white font-medium transition-colors pointer-events-auto"
                ...
                <span className="text-xs text-slate-500 tracking-wider pointer-events-auto">Explore →</span>
              )}
            </div>
