...
export function DetailPage({ identity }: DetailPageProps) {
  return (
    <main className="min-h-screen bg-black">
      ...
      <section className="relative h-screen">
        ...
        <div className="relative z-10 flex items-center h-full px-8 md:px-16 lg:px-24">
          <div className="max-w-3xl space-y-6">
            <div className="space-y-2">
              <p className="text-slate-400 tracking-[0.3em] text-sm uppercase">{identity.ageRange}</p>
              <h1 className="font-serif text-7xl md:text-8xl lg:text-9xl tracking-tighter text-slate-50">
                {identity.title}
              </h1>
              <p className="text-2xl md:text-3xl text-slate-300 tracking-wide font-light">{identity.tagline}</p>
            </div>
          </div>
        </div>
        ...
        {/* WhatsApp Button */}
        <a
          href="https://wa.me/1234567890?text=Hi%2C%20I'm%20interested%20in%20dance%20classes"
          target="_blank"
          rel="noopener noreferrer"
      ...
      <section className="py-24 px-8 md:px-16 lg:px-24 bg-zinc-950">
        <div className="max-w-4xl mx-auto">
          ...
          <p className="text-lg md:text-xl text-slate-400 leading-relaxed">{identity.overview}</p>
        </div>
      </section>
      ...
      <section className="py-24 px-8 md:px-16 lg:px-24 bg-black">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {identity.danceStyles.map((style) => (
              <Link
                key={style.name}
                href={style.link}
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  <ExternalLink className="w-5 h-5 text-slate-400 group-hover:text-slate-50 transition-colors" />
                </div>
                <p className="text-slate-400 leading-relaxed">{style.description}</p>
              </Link>
            ))}
          </div>
        </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="p-8 rounded-2xl bg-white/5 border border-white/10">
              <div className="mb-6">
                ...
                <div className="flex items-baseline gap-2">
                  <span className="font-serif text-4xl text-slate-50">{identity.pricing.groupClasses.price}</span>
                  <span className="text-slate-400 text-sm">{identity.pricing.groupClasses.duration}</span>
                </div>
              </div>
              <p className="text-slate-400 text-sm leading-relaxed">{identity.pricing.groupClasses.description}</p>
            </div>
            ...
            <div className="p-8 rounded-2xl bg-white/5 border border-white/10">
              <div className="mb-6">
                ...
                <div className="flex items-baseline gap-2">
                  <span className="font-serif text-4xl text-slate-50">{identity.pricing.semiPrivate.price}</span>
                  <span className="text-slate-400 text-sm">{identity.pricing.semiPrivate.duration}</span>
                </div>
              </div>
              <p className="text-slate-400 text-sm leading-relaxed">{identity.pricing.semiPrivate.description}</p>
            </div>

            {/* Private Classes */}
            <div className="p-8 rounded-2xl bg-white/5 border border-white/10 lg:border-white/20">
              <div className="mb-6">
                <h3 className="text-slate-400 text-sm tracking-widest uppercase mb-3">Private Classes</h3>
                <div className="flex items-baseline gap-2">
                  <span className="font-serif text-4xl text-slate-50">{identity.pricing.privateClasses.price}</span>
                  <span className="text-slate-400 text-sm">{identity.pricing.privateClasses.duration}</span>
                </div>
              </div>
              <p className="text-slate-400 text-sm leading-relaxed">{identity.pricing.privateClasses.description}</p>
            </div>
            ...
            <div className="p-8 rounded-2xl bg-white/5 border border-white/10">
              <div className="mb-6">
                ...
                <div className="flex items-baseline gap-2">
                  <span className="font-serif text-4xl text-slate-50">{identity.pricing.dropIn.price}</span>
                  <span className="text-slate-400 text-sm">{identity.pricing.dropIn.duration}</span>
                </div>
              </div>
              <p className="text-slate-400 text-sm leading-relaxed">{identity.pricing.dropIn.description}</p>
            </div>
          </div>
        </div>
      </section>
